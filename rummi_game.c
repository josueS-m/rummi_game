...
#define FICHAS_INICIALES 14 // Cartas al repartir (puede variar según reglas)
...
typedef struct
{
    ...
    int fichas_en_mano;         // Cartas actuales
    ...
    int fichas_robadas;         // Cartas robadas
    int fichas_desfichadas;     // Cartas desfichadas
...
int num_de_esperas = 0;                               // Contador de jugadores de_esperas
...
// Función para que el jugador intente embonar todas las fichas posibles
void jugador_embonar_fichas(jugador_t *jugador, banco_de_apeadas_t *banco) {
    bool embonada_algo = true;

    while (embonada_algo) {
        embonada_algo = false;

        // Intentar embonar cada ficha (de atrás hacia adelante para evitar problemas de índices)
        for (int i = jugador->mano.cantidad - 1; i >= 0; i--) {
            if (embonar_ficha(jugador, banco, i)) {
                embonada_algo = true;
...
void mostrar_mano(mano_t *mano)
{
    ...
    for (int i = 0; i < mano->cantidad; i++)
    {
        ...
        {
            printf("%d de %s\n", mano->fichas[i].numero, mano->fichas[i].color);
        }
    }
}
...
void actualizar_pcb(pcb_t *pcb, jugador_t *jugador) {
    ...
    pcb->fichas_robadas = 0;
    pcb->fichas_desfichadas = 0;
...
void escribir_pcb(pcb_t jugador)
{
    ...
    fprintf(file, "Cartas Desfichadas: %d\n", jugador.fichas_desfichadas);
...
void *jugador_thread(void *arg)
{
    ...
    while (turno_activo && !juego_terminado)
    {
        ...
        case 5:
            if (!jugador->ficha_agregada && mazo.cantidad > 0)
            {
                ficha_t nueva = mazo.fichas[--mazo.cantidad];
                agregar_ficha(&jugador->mano, nueva);
                mostrar_robo_ficha(&nueva, true);
                mi_pcb->fichas_robadas++;
            }
            turno_activo = false;
            break;
...
void *ejecutarFCFS(void *arg)
{
    ...
    if (jugador->en_juego)
    {
        printf("\nJugador %d juega su turno completo.\n", jugador->id);
...
{
    const char *politica = (modo == 'F') ? "FCFS (Turnos completos en orden de llegada)" : "Round Robin (Quantum de %d segundos)";
